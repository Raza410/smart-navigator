        ...
        L.Control.Measure = L.Control.extend({
            ...
            _buildItems: function () {
                ...
                L.DomEvent.on(
                    ele_link_line,
                    "click",
                    this._enableMeasureLine.bind(this)
                );

                ele_li = L.DomUtil.create("li", "leaflet-measure-action", ele_ul);
                let ele_link_area = L.DomUtil.create(
                    "a",
                ...
                L.DomEvent.on(
                    ele_link_area,
                    "click",
                    this._enableMeasureArea.bind(this)
                );
            },
        ...
        L.MeasureLable = L.Layer.extend({
            ...
            enableClose: function () {
                ...
                this._closeButton.innerHTML =
                    '<svg className="icon" viewBox="0 0 40 40"><path stroke="#FF0000" stroke-width="3" d="M 10,10 L 30,30 M 30,10 L 10,30" /></svg>';
...
const MeaureTool = ({ onLoaded }) => {
    const map = useMap();

    useEffect(() => {
        if (!map) return;
        if (!L.control.measure) {
            console.log('L.control.measure is not defined');
        }
        ...
        // Notifying the Map page that MeasureTool has loaded to load the fullscreen button after it
        if (onLoaded) {
            onLoaded();
        }
