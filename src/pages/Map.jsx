...
function Map({ children, style }) {
  ...
  const onEachBuilding = (feature, layer) => {
    if (feature.properties && feature.properties.name) {
      layer.bindPopup(`
        <h3 style="text-align: center; background-color:#C0C0C0; margin-top:5px; margin-bottom:5px; padding: 10px; border-radius: 4px;">
      ...
      layer.on("click", (event) => {
        ...
        layer.bindPopup(`
          <h2>${event.target.feature.properties.Name}</h2><br>
          <img src="${event.target.feature.properties.Departments[0].Image}" alt="${event.target.feature.properties.Name}" />
        `).openPopup();
      });
      ...
      layer.on("popupopen", (event) => {
        ...
        if (detailsLink) {
          detailsLink.onclick = () => {
            setSelectedBuilding(event.target.feature.properties);
          };
        }
      });
    }
